...
# Configuration
CONFIG = {
    "BOT_TOKEN": "8109103757:AAF5fYbLO24zEk7WxSJufH1hORVMdwacMog",
    "IMG_API_URL": "https://img-gen.hazex.workers.dev/",
...
def load_users_from_file():
    """Load user IDs from users.txt into the database if the file exists."""
    ...
    try:
        ...
        for user_id in user_ids:
            manage_user_data(user_id)  # Inserts if not exists
    ...
    try:
        with filename.open("rb") as f:
            await context.bot.send_document(
                chat_id=user_id,
                document=f,
                caption=lang["num_users"].format(num_users, user_ids)
            )
...
async def handle_callback_query(update: Update, context: ContextTypes) -> None:
    ...
    if query.data.startswith("dim_"):
        ...
        # Removed send_to_topic for improve prompt
        return
